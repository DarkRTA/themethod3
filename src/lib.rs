...
fn gen_key(hv_key: &[u8; 16], mogg_data: &[u8], version: u32) -> [u8; 16] {
    ...
    debug!("keyMaskPS3: {}", hex::encode(key_mask_ps3));
    debug!("keyMask360: {}", hex::encode(key_mask_360));
    ...
    debug!("selectedKeyPS3: {}", hex::encode(selected_key_ps3));
    debug!("selectedKey360: {}", hex::encode(selected_key_360));

    let revealed_key_ps3 = reveal_key(selected_key_ps3, masher);
    let revealed_key_360 = reveal_key(selected_key_360, masher);

    debug!("revealedKeyPS3: {}", hex::encode(selected_key_ps3));
    debug!("revealedKey360: {}", hex::encode(selected_key_360));
    ...
    debug!("revealedKeyPS3 char: {}", hex::encode(bytes_from_hex_string_ps3));
    debug!("revealedKey360 char: {}", hex::encode(bytes_from_hex_string_360));
    ...
    debug!("grind_array_result_PS3: {}", hex::encode(grind_array_result_ps3));
    debug!("grind_array_result_360: {}", hex::encode(grind_array_result_360));
    ...
    debug!("ps3_key: {}", hex::encode(ps3_key));
    debug!("360_key: {}", hex::encode(x360_key));
    x360_key
}
